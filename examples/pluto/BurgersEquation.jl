...
md"""
## Tutorial notebook: Burger's equation

This example solves the one-dimensional (viscous) Burger's equation
```math
\begin{aligned}
u_t - \mu \Delta u + \mathrm{div} f(u) & = 0
\end{aligned}
```
on the interval ``(-2,2)`` with periodic boundary conditions, ``f(u) = \frac{1}{2} u^2`` and some initial profile ``u(0) = u_0``.
...
begin
	## PROBLEM DATA
	f = (result, u) -> (result[1] = u[1]^2/2;) # kernel for nonlinearity
	u0 = DataFunction((result,x) -> (result[1] = abs(x[1]) < 0.5 ? 1 : 0), [1, 1]; dependencies = "X", bonus_quadorder = 10)  # initial state
	ν = 1e-2
    ...
    add_constraint!(Problem, CombineDofs(1, 1, [1], [num_nodes(grid)]))
	@info Problem
end
...
begin
	## FINITE ELEMENT SPACES
	order = 2
	FES = FESpace{H1Pk{1,1,order}}(grid)
	sol = FEVector(FES)
	@info sol
end
...
scalarplot(grid, sol4t[t+1]; limits = (-0.1,1.1), levels = 1, title = "c ( T=$(Float64(t*τ)) )")
...
deps = ["CommonSolve", "DiffResults", "DocStringExtensions", "ExtendableGrids", "ExtendableSparse", "ForwardDiff", "GridVisualize", "LinearAlgebra", "LinearSolve", "Logging", "Printf", "SparseArrays", "SparseDiffTools", "StaticArrays", "SuiteSparse", "Symbolics", "Test"]
git-tree-sha1 = "8291a8f11e480b720bc712114a6bb9a25eed8649"
