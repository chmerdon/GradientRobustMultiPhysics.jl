...
md"""
# Tutorial notebook: inf-sup stabilized divergence-free Scott--Vogelius 

Seek velocity ``u`` and pressure ``p`` such that
```math
\begin{aligned}
	- \mu \Delta u + \nabla p & = f \\
       - \mathrm{div}(u) & = 0
\end{aligned}
```
with given data on a given domain ``\Omega``.

The weak formulation seeks ``(u,p) \in H^1(\Omega)^2 \times L^2_0(\Omega)`` such that
```math
\begin{aligned}
	(\mu \nabla u, \nabla v) - (\mathrm{div} v, p) & = (f, v) && \quad \text{for all } v \in H^1_0(\Omega),\\
(\mathrm{div} u, q) & = 0 && \quad \text{for all } q \in L^2_0(\Omega).
\end{aligned} 
```

This notebook experiments with different variants of the Scott-Vogelius finite element method. In its classical version it is only guaranteed to be inf-sup stable on certain classes of meshes, like barycentric refined ones. Without inf-sup stability one cannot expect unique solvability.

On general meshes a novel Raviart-Thomas enrichment can be used to guarantee inf-sup stability while preserving pressure-robustness. Here, we also compare it to a Taylor-Hood method that also works on general meshes, but is not pressure-robust and therefore shows velocity errors that scale with ``1/\mu``.


!!! reference

    ''Inf-sup stabilized Scott--Vogelius pairs on general simplicial grids by Raviart--Thomas enrichment'', Volker John, Xu Li, Christian Merdon, Hongxing Rui (arxiv preprint no. 2206.01242)
    [>Link<](https://arxiv.org/abs/2206.01242)
...
md"""
	The following plots show the grid, the absolute value of the velocity field and the pressure:
"""

# ╔═╡ 2f89775a-37e0-4cf8-b23c-d8339d09a0d1
md"""
	The following plots show the divergence of the H1 velocity field and (if applicable) the divergence of the H1 velocity field + RT enrichment:
"""

# ╔═╡ 765352bb-c097-4ceb-8ed1-ded021ab6218
begin
	## generate grid
	function get_grid(nref)
		return simplexgrid(Triangulate;
...
[[deps.GradientRobustMultiPhysics]]
deps = ["DiffResults", "DocStringExtensions", "ExtendableGrids", "ExtendableSparse", "ForwardDiff", "GridVisualize", "LinearAlgebra", "Logging", "Printf", "SparseArrays", "SparseDiffTools", "StaticArrays", "SuiteSparse", "Symbolics", "Test"]
git-tree-sha1 = "3bef626b089766e5490675518708dd225ca66efa"
