...
function assemble_operator!(b,::Type{FACE_L2_JumpDA},FE::AbstractH1FiniteElement, dofs4A)
    ...
    gradients = zeros(Float64,FEbasis[1].ndofs4item,FEbasis[1].ncomponents*FEbasis[1].xdim);
          
    # quadrature loop
    jump4face4qp = zeros(Float64,length(qf.xref),FEbasis[1].xdim*FEbasis[1].ncomponents);
    cell = 0
    nac = 0
    pos = 
    #@time begin
        ...
        if nac == 2
            ...
            for ac = 1 : nac
                ...
                for i in eachindex(qf.w) # loop over all quadrature points on face (masked as cell points)
                    ...
                    FiniteElements.getFEbasisgradients4qp!(gradients, FEbasis[pos[1]], i)
                    ...
                    for dof_i = 1 : FEbasis[pos[1]].ndofs4item
                        ...
                        for k = 1 : size(gradients,2)
                            if (ac == 1)
                                jump4face4qp[i,k] += gradients[dof_i,k] * dofs4A[FEbasis[pos[1]].current_dofs[dof_i]];
                            else
                                jump4face4qp[length(qf.xref)+1-i,k] -= gradients[dof_i,k] * dofs4A[FEbasis[pos[1]].current_dofs[dof_i]];
                            end    
                        end
                    end
                end  
