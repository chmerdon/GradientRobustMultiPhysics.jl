...
function assemble_operator!(b,::Type{FACE_JDAdotJDA},FE::AbstractH1FiniteElement,  dofs4A)
    ...
    # quadrature loop
    jump4face4qp = zeros(Float64,length(qf.xref),xdim*ncomponents);
    cell = 0
    nac = 0
    pos = 
    #@time begin
                ...
                for i in eachindex(qf.w) # loop over all quadrature points on face (masked as cell points)
                    ...
                    for dof_i = 1 : ndofs4cell
                        ...
                        for k = 1 : size(gradients,2)
                            if (ac == 1)
                                jump4face4qp[i,k] += gradients[dof_i,k] * dofs4A[dofs[dof_i]];
                            else
                                jump4face4qp[length(qf.xref)+1-i,k] -= gradients[dof_i,k] * dofs4A[dofs[dof_i]];
                            end    
                        end
                    end
                end  
