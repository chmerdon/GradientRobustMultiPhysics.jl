struct H1MINIFiniteElement{T,dim,ncomponents} <: AbstractH1FiniteElement where {T <: Real, ncomponents <: Int}
    name::String;                 # full name of finite element (used in messages)
    grid::Grid.Mesh{T};           # link to grid
end

# MINI needs a dim-dependeny, since cell bubbles looks different
#      and causes different get_basis_on_elemtype

function getMINIFiniteElement(grid,dim,ncomponents)
    ensure_nodes4faces!(grid);
    ensure_volume4cells!(grid);
    T = eltype(grid.coords4nodes);
    @assert dim > 1 # dim=1 makes no sense (or is the same as P2)
    return H1MINIFiniteElement{T,dim,ncomponents}("MINI (H1FiniteElement, dim=$dim, ncomponents=$ncomponents)",grid)
end 
...
get_ndofs4elemtype(FE::H1MINIFiniteElement{T,2,1} where {T <: Real}, ::Grid.Abstract1DElemType) = 2
get_ndofs4elemtype(FE::H1MINIFiniteElement{T,2,1} where {T <: Real}, ::Grid.ElemType2DTriangle) = 4
get_ndofs4elemtype(FE::H1MINIFiniteElement{T,2,2} where {T <: Real}, ::Grid.Abstract1DElemType) = 4
get_ndofs4elemtype(FE::H1MINIFiniteElement{T,2,2} where {T <: Real}, ::Grid.ElemType2DTriangle) = 8
...
function get_dofs_on_cell!(dofs,FE::H1MINIFiniteElement{T,2,1} where {T <: Real}, cell::Int64, ::Grid.ElemType2DTriangle)
    dofs[1:3] = FE.grid.nodes4cells[cell,:]
    dofs[4] = size(FE.grid.coords4nodes,1) + cell
end
function get_dofs_on_cell!(dofs,FE::H1MINIFiniteElement{T,2,2} where {T <: Real}, cell::Int64, ::Grid.ElemType2DTriangle)
    dofs[1:3] = FE.grid.nodes4cells[cell,:]
    dofs[4] = size(FE.grid.coords4nodes,1) + cell
    dofs[5:8] = (size(FE.grid.coords4nodes,1) + size(FE.grid.nodes4cells,1)) .+ dofs[1:4]
end

function get_dofs_on_face!(dofs,FE::H1MINIFiniteElement{T,2,1} where {T <: Real}, face::Int64, ::Grid.Abstract1DElemType)
    dofs[:] = FE.grid.nodes4faces[face,:]
end
function get_dofs_on_face!(dofs,FE::H1MINIFiniteElement{T,2,2} where {T <: Real}, face::Int64, ::Grid.Abstract1DElemType)
