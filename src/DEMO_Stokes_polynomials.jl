...
function main()
    ...
    for level = 1 : maxlevel
        ...
        # compute errors
        L2error_pressure[level] = sqrt(FESolveCommon.assemble_operator!(FESolveCommon.DOMAIN_L2_FplusA,exact_pressure!,FE_pressure ,val4dofs[ndofs_velocity+1:end]; degreeF = polynomial_order, factorA = -1.0))
        L2error_velocity[level] = sqrt(FESolveCommon.assemble_operator!(FESolveCommon.DOMAIN_L2_FplusA,exact_velocity!,FE_velocity ,val4dofs; degreeF = polynomial_order-1, factorA = -1.0))
