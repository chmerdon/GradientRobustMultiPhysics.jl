mutable struct FEBasisEvaluator{T <: Real, FE <: AbstractFiniteElement, EGEG <: AbstractElementGeometry, FEOP <: AbstractFEFunctionOperator}
...
function FEBasisEvaluator{T,FEType,EG,FEOP}(FE::AbstractFiniteElement, qf::QuadratureRule, AT::Type{<:AbstractAssemblyType}; force_updateL2G = false) where {T <: Real, FEType <: AbstractFiniteElement, EG <: AbstractElementGeometry, FEOP <: AbstractFEFunctionOperator}
    ...
    if derivorder > 0
        ...
        for i in eachindex(qf.w)
            ...
            current_eval[i] = zeros(T,ndofs4item,ncomponents*xdim)
        end 
    end
    if derivorder > 1
        refoperatorvals = Array{Array{T,2},1}(undef,length(qf.w));
        ...
        for i in eachindex(qf.w)
            ...
            current_eval = zeros(T,ndofs4item,ncomponents*xdim*xdim);
        end   
    end
    if derivorder == 0
    ...
    return FEBasisEvaluator{T,FEType,EG,FEOP}(ItemDofs,L2G,L2GM,force_updateL2G,qf.xref,refbasisvals,refoperatorvals,ncomponents,offsets,offsets2,0,current_eval)
end    


# IDENTITY OPERATOR
# H1 ELEMENTS (nothing has to be done)
function update!(FEBE::FEBasisEvaluator{T,FE,EG,FEOP}, item::Int) where {T <: Real, FE <: AbstractH1FiniteElement, EG <: AbstractElementGeometry, FEOP <: Identity}
    FEBE.citem = item
...
function update!(FEBE::FEBasisEvaluator{T,FE,EG,FEOP}, item::Int) where {T <: Real, FE <: FiniteElements.AbstractH1FiniteElementWithCoefficients, EG <: AbstractElementGeometry, FEOP <: Identity}
    FEBE.citem = item
...
function update!(FEBE::FEBasisEvaluator{T,FE,EG,FEOP}, item::Int) where {T <: Real, FE <: AbstractH1FiniteElement, EG <: AbstractElementGeometry, FEOP <: Gradient}
    FEBE.citem = item
    ...
    for i = 1 : length(FEBE.cvals)
        if FEBE.L2G.nonlinear || i == 1
            tinvert!(FEBE.L2GM,FEBE.L2G,FEBE.xref[i])
        end
        for dof_i = 1 : FEBE.offsets2[2]
            for c = 1 : FEBE.ncomponents, k = 1 : FEBE.offsets[2]
                FEBE.cvals[i][dof_i,k + FEBE.offsets[c]] = 0.0;
                for j = 1 : FEBE.offsets[2]
                    # compute duc/dxk
