...
#use_problem = "quadratic"; u_order = 2; error_order = 4; p_order = 1; f_order = 1;
...
function exact_pressure(problem) # exact pressure
    function closure(x)
        ...
        elseif problem == "quadratic"
            return x[1] - 1//2;    
        elseif problem == "cubic"
            return x[1]^2 + x[1]^2 - 2//3;    
        end
    end    
end


function volume_data!(problem) # volume data (generated by FowardDiff)
...
show(minimum(val4dofs[ndofs_velocity+1:end]))
show(maximum(val4dofs[ndofs_velocity+1:end]))
