...
include("PROBLEMdefinitions/STOKES_potentialflow.jl");
...
function main()
    ...
    # load problem data
    PD, exact_velocity!, exact_pressure! = getProblemData(order_potential_flow, nonlinear, 4);
    PD.viscosity = nu;
    ...
    for level = 1 : maxlevel
        ...
        # compute errors
        L2error_pressure[level] = sqrt(FESolveCommon.assemble_operator!(FESolveCommon.DOMAIN_L2_FplusA,exact_pressure!,FE_pressure ,val4dofs[ndofs_velocity+1:end]; degreeF = 3, factorA = -1.0))
        L2error_velocity[level] = sqrt(FESolveCommon.assemble_operator!(FESolveCommon.DOMAIN_L2_FplusA,exact_velocity!,FE_velocity ,val4dofs; degreeF = 7, factorA = -1.0))
